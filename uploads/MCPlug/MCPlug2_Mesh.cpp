...
void MCplug2::export_mesh(INode* node, TimeValue t, mesh_data* mesh_node)
{
	...
	for (i = 0; i < point_num; i++) {
		...
		mesh_node->m_point_list[i].x = v.x;
		mesh_node->m_point_list[i].y = v.z;
		mesh_node->m_point_list[i].z = v.y;
	}

	for (i = 0; i < face_num; i++) {
		mesh_node->m_face_list[i].m_point_index[0] = mesh->faces[i].v[vx3];
	...
	if (numCVx) {
		...
		mesh_node->m_point_color_list = new D3DXVECTOR3[point_num];
