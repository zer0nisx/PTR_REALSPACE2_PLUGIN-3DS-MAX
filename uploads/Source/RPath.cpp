...
bool RPathList::Save(const char *filename,int nSourcePolygon)
{
	...
	int nSize=size();
	fwrite(&nSourcePolygon,sizeof(int),1,file);		
	...
	for(iterator i=begin();i!=end();i++)
	{
		...
		for(j=0;j<node->vertices.size();j++)
		{
			...
			if(nCounts[j]>1){	
				...
				_ASSERT(!IS_ZERO(dir.x) || !IS_ZERO(dir.y));		
...
void RPathList::MarkGroupID(RPathNode *pNode,int nGroupID)			
{
