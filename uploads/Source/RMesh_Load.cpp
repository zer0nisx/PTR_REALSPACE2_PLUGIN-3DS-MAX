...
#define MZF_READ(x,y) { if(!mzf.Read((x),(y))) return false; }
	...
	for (i = 0; i < t_hd.mtrl_num; i++) {
		...
		// ===== EXPORTER_MESH_VER9 - PBR Material Loading =====
		if (t_hd.ver >= EXPORTER_MESH_VER9)
		{
			// Convert to RMtrl_V9 for extended properties
			RMtrl_V9* nodeV9 = new RMtrl_V9();

			// Copy basic properties from legacy material
			nodeV9->m_mtrl_id = node->m_mtrl_id;
			nodeV9->m_sub_mtrl_id = node->m_sub_mtrl_id;
			nodeV9->m_ambient = node->m_ambient;
			nodeV9->m_diffuse = node->m_diffuse;
			nodeV9->m_specular = node->m_specular;
			nodeV9->m_power = node->m_power;
			nodeV9->m_sub_mtrl_num = node->m_sub_mtrl_num;
			nodeV9->m_bTwoSided = node->m_bTwoSided;
			nodeV9->m_bAdditive = node->m_bAdditive;
			nodeV9->m_bAlphaTestMap = node->m_bAlphaTestMap;
			nodeV9->m_nAlphaTestValue = node->m_nAlphaTestValue;
			strcpy(nodeV9->m_name, node->m_name);
			strcpy(nodeV9->m_opa_name, node->m_opa_name);

			// Read extended PBR texture paths
			MZF_READ(nodeV9->m_normal_map, MAX_PATH_NAME_LEN);
			MZF_READ(nodeV9->m_specular_map, MAX_PATH_NAME_LEN);
			MZF_READ(nodeV9->m_roughness_map, MAX_PATH_NAME_LEN);
			MZF_READ(nodeV9->m_metallic_map, MAX_PATH_NAME_LEN);
			MZF_READ(nodeV9->m_emissive_map, MAX_PATH_NAME_LEN);
			MZF_READ(nodeV9->m_ao_map, MAX_PATH_NAME_LEN);
			MZF_READ(nodeV9->m_height_map, MAX_PATH_NAME_LEN);
			MZF_READ(nodeV9->m_reflection_map, MAX_PATH_NAME_LEN);
			MZF_READ(nodeV9->m_refraction_map, MAX_PATH_NAME_LEN);

			// Read PBR properties
			MZF_READ(&nodeV9->m_roughness, sizeof(float));
			MZF_READ(&nodeV9->m_metallic, sizeof(float));
			MZF_READ(&nodeV9->m_ior, sizeof(float));
			MZF_READ(&nodeV9->m_emissive_intensity, sizeof(float));
			MZF_READ(&nodeV9->m_emissive, sizeof(D3DXCOLOR));
			MZF_READ(&nodeV9->m_material_type, sizeof(int));

			// Read UV channel assignments
			MZF_READ(&nodeV9->m_uv_diffuse, sizeof(int));
			MZF_READ(&nodeV9->m_uv_normal, sizeof(int));
			MZF_READ(&nodeV9->m_uv_specular, sizeof(int));
			MZF_READ(&nodeV9->m_uv_roughness, sizeof(int));
			MZF_READ(&nodeV9->m_uv_metallic, sizeof(int));
			MZF_READ(&nodeV9->m_uv_emissive, sizeof(int));
			MZF_READ(&nodeV9->m_uv_ao, sizeof(int));
			MZF_READ(&nodeV9->m_uv_height, sizeof(int));

			// Read UV transformations
			MZF_READ(nodeV9->m_uv_tiling, sizeof(D3DXVECTOR2) * 8);
			MZF_READ(nodeV9->m_uv_offset, sizeof(D3DXVECTOR2) * 8);
			MZF_READ(nodeV9->m_uv_rotation, sizeof(float) * 8);

			// Read texture flags
			MZF_READ(&nodeV9->m_bHasNormalMap, sizeof(bool));
			MZF_READ(&nodeV9->m_bHasSpecularMap, sizeof(bool));
			MZF_READ(&nodeV9->m_bHasRoughnessMap, sizeof(bool));
			MZF_READ(&nodeV9->m_bHasMetallicMap, sizeof(bool));
			MZF_READ(&nodeV9->m_bHasEmissiveMap, sizeof(bool));
			MZF_READ(&nodeV9->m_bHasAOMap, sizeof(bool));
			MZF_READ(&nodeV9->m_bHasHeightMap, sizeof(bool));

			// Set texture flags based on file names
			if (strlen(nodeV9->m_normal_map) > 0) {
				nodeV9->m_bHasNormalMap = true;
			}
			if (strlen(nodeV9->m_specular_map) > 0) {
				nodeV9->m_bHasSpecularMap = true;
			}
			if (strlen(nodeV9->m_roughness_map) > 0) {
				nodeV9->m_bHasRoughnessMap = true;
			}
			if (strlen(nodeV9->m_metallic_map) > 0) {
				nodeV9->m_bHasMetallicMap = true;
			}
			if (strlen(nodeV9->m_emissive_map) > 0) {
				nodeV9->m_bHasEmissiveMap = true;
			}
			if (strlen(nodeV9->m_ao_map) > 0) {
				nodeV9->m_bHasAOMap = true;
			}
			if (strlen(nodeV9->m_height_map) > 0) {
	...
	for (i = 0; i < t_hd.mesh_num; i++) {
		...
		if (pMeshNode->m_face_num) {
			...
			else if (t_hd.ver > EXPORTER_MESH_VER2) {
				MZF_READ(pMeshNode->m_face_list, sizeof(RFaceInfo) * pMeshNode->m_face_num);
			}
			else {
